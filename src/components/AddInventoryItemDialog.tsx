...
import {
  RecipePriceCalculationDialog,
  type RecipeData,
} from "@/components/RecipePriceCalculationDialog"
...
interface AddInventoryItemDialogProps {
  open: boolean
  onClose: () => void
  onSubmit?: (values: AddItemValues, recipe?: RecipeData) => void
}
...
export function AddInventoryItemDialog({
  ...
  const [pendingValues, setPendingValues] = useState<AddItemValues | null>(null)
  ...
  function handleRecipeSave(recipe: RecipeData) {
    if (onSubmit && pendingValues) {
      onSubmit(pendingValues, recipe)
    }
  ...
  function handleRecipeClose() {
    // User dismissed the recipe dialog without saving — still add the item
    if (onSubmit && pendingValues) {
      ...
      {pendingValues && (
        <RecipePriceCalculationDialog
          ...
          initialIngredient={{
            ...
            costPerUnit: pendingValues.costPerUnit,
            initialStock: pendingValues.initialStock,
          }}
        />
      )}
    </>
